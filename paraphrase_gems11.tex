...
to derive feature functions and extract paraphrase tables. Feature functions and the language model feature are combined with feature weights in a log-linear model. The paraphrase that maximizes the log-linear function is then selected as the optimal paraphrase. Feature weights are optimized by minimizing the phrase substitution error rate, an objective function proposed by the authors, on the development set using gradient descent. In our work, we examine how bilingual and monolingual re-ranking metric complement each other and explore the possibilities of robustly combining the two scores in the future.
