...
% Mono: meaning and grammar fairly consistent in correlation measure except for SyntBiPmatched. can be due to the fact that most  paraphrase candidates that can possible be grammatical were eliminated in the synt type matching process, reducing the number of ranked candidates,...
...
The quality of paraphrase ranking can be reflected through the relationship between the averaged human scores and the top K candidates as K increases. Figure \ref{fig_meanTopK} shows the mean human assigned score within the top K candidates averaged across all phrases. Compared across the categories, meaning scores have lower range of score and a more uniform trend of decreasing values as a function of K.  In grammaticality, BiP clearly underperforms whereas the SyntBiP-MonoDS maintains the best score among all methods over all values of K. In addition, a slow dropoff up until K = 4 implies that quality of paraphrases remains relatively high going from top 1 to top 4 candidates.
...
% affected by context
%- fiscal burden   918627  tax     5.5     3.5     16.0    5.0 (m:18)==> correct based on context not by definition, example not as "clean" (not top or bottom rankings)
% hauled  344042  delivered       17.0    18.5    2.5     19.0
...
{\scriptsize \emph{fiscal burden, tax}} & {\scriptsize 18} & {\scriptsize 13.5} & {\scriptsize 15.5} & {\scriptsize 3} & {\scriptsize 14} \\
...
\end{tabular}
\end{center}
\caption{Examples of phrase pair rankings by different re-ranking methods and human judgments in terms of meaning and grammar.}
\label{table11}
\end{table*}
